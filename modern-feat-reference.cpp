...
{	
	...
	f(0);		//calls f(int)
//!	f(NULL);	//calls f(int) if NULL is 0, ambiguous otherwise   	//[Warning] passing NULL to non-pointer argument 1 of 'void f(int)' [-Wconversion-null]
...
void test06_lambda()
{
    ...
    cout << lf(9,8) << endl;    //72
//! decltype(lf)(3,5);	//[Error] no matching function for call to 'std::function<int(int, int)>::function(int, int)'
...
//! decltype(f)(3,5);	//[Error] no matching function for call to 'jj06::test06_lambda()::<lambda()>::__lambda9(int, int)'
//! decltype(f)();		//[Error] use of deleted function 'jj06::test06_lambda()::<lambda()>::<lambda>()'
...
//!	typename tuple<int, float, string> TupleType;			//[Error] expected nested-name-specifier   [Error] expected '('
//!	cout << tuple_size<TupleType>::value << endl;	  		//yields 3
//!	tuple_element<1, TupleType>::type  PI { 3.141592653 }; 	//yields float
...
//!      cout << get<i>(t4);   //[Error] the value of 'i' is not usable in a constant expression
    ...
    cout << make_tuple(7.5, string("hello"), bitset<16>(377), 42); //provided there is a overloaded operator<< for bitset. 
}
}
...
//!	array<int,10> c2 = { 42 };   	//[Error] expected primary-expression before 'int'	     
    ...
    for(const auto& s: c2)
        std::cout << s << ' ';		// 42 0 0 0 0 0 0 0 0 0 
    std::cout << '\n';
...
void test15_variadic_template()
{
	...
	tuple<string, complex<int>, bitset<16>, double> 
	    t2("Ace", complex<int>(3,8), bitset<16>(377), 3.141592653);
...
void test23_traits_for_string()
{
	...
	cout << "iterator_traits<set<string>::iterator>::value_type ==> " << typeid(T).name() << endl;	//Ss (奇怪, 只有縮寫)
...
//! int& c(5);			//[Error] invalid initialization of non-const reference of type 'int&' from an rvalue of type 'int'
//! process(move(c));	//上行失敗本行當然不必說, "c" was not declared in this scope 
}
}
//----------------------------------------------------
namespace jj28
...
public:
  ...
  Foo& operator=(const Foo&) {  cout << "Foo Copy Assignment \n";   return *this;  }  
  ~Foo() { cout << "~Foo()" << endl; }
};
...
{
public:
  T() : intArray { 6,5,4,3,2,1 }   { 	//[Warning] list-initializer for non-class type must not be parenthesized
  }										//上述, 可拿掉小括弧 (已測試ok) 
  print() {
...
void test45_default_and_delete()
{
	...
	f3 = f2;	//copy assign (如果 copy ctor = delete; [Error] use of deleted function 'Foo& Foo::operator=(const Foo&)')
